...
system.time(for (year in Year.vec[11:20]) {
  
  if (reduce_catch == F) {
    catch.by.fleet[1] <- runif(1, c_1[1], c_1[2])
    catch.by.fleet[2] <- runif(1, c_2[1], c_2[2])
    catch.by.fleet[3] <- runif(1, c_3[1], c_3[2])
    catch.by.fleet[4] <- runif(1, c_4[1], c_4[2])
  ...
  if (reduce_catch == T) {
    f.by.fleet[1] <- rlnorm(1, log(f.reduced[1]), 0.1)
    f.by.fleet[2] <- rlnorm(1, log(f.reduced[2]), 0.1)
    f.by.fleet[3] <- rlnorm(1, log(f.reduced[3]), 0.1)
    dis <- rnorm(1, C_4_mu, 50)
    f.by.fleet[4] <- solve_for_f(dis, 4, dat.list, year - 1, N)
    f.by.fleet[5] <- proj.index[year - 1, 2]
  }
  ...
  .datcatch[year, c(1:2)] <-
    apply(catch.in.biomass(dat.list, N, year, z, f.by.fleet)[c(1:2), ], 1, sum)
  ...
  if (year %% 10 == 0) {
    ...
    if (rebuild == F) {
      ...
      F_1_mu <- solve_for_f(ABC, 1, dat.list, year, N)
      F_1_mu <- log(F_1_mu)
      F_2_mu <- solve_for_f(ABC, 2, dat.list, year, N)
      F_2_mu <- log(F_2_mu)
      F_3_mu <- solve_for_f(ABC, 3, dat.list, year, N)
      F_3_mu <- log(F_3_mu)
    }
