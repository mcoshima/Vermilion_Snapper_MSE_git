...
system.time(for (year in Year.vec[1:5]) {
  ...
  if (recommend_catch == F) {
    ...
    f.by.fleet[5] <- proj.index$RS_relative[year/2] * slope + intercept
    
  }
  
  if (recommend_catch == T) {
    .datcatch[year,1] <- rnorm(1, c_1, 300)
    .datcatch[year,2] <- rnorm(1, c_2, 100)
    .datcatch[year,3] <- rnorm(1, c_3, 300)
  ...
  #Catches
  catch.by.fleet[c(1:3,5), ] <-
    catch.in.number(dat.list, N, year, z, f.by.fleet[c(1:3,5)])[c(1:3,4),]
  catch.by.fleet[4, ] <- as.numeric((asel[4, ] / sum(asel[4, ])) * .datcatch[year,4])
